...
def get_graph(exp, subset, out_dir):
    ...
    my_graph = Path(out_dir)/f'{exp}_graph_{subset}_leiden.pkl'
...
def cluster_leiden(Gm, df_peaks, res, fig_dir, exp, subset):
    ...
    if res == 'auto':
        ...
        plt.savefig(f'{fig_dir}/{exp}_clust_res_var_{subset}.jpeg')
...
def plot_clusters(df_peaks, res, fig_dir, exp, subset):
    """Plots clusters.
    ...
    fig.write_html(f'{fig_dir}/{exp}_clust_leiden_{subset}.html')
    fig.write_image(f'{fig_dir}/{exp}_clust_leiden_{subset}.jpeg')
    fig.write_image(f'{fig_dir}/{exp}_clust_leiden_{subset}.svg')
...
if __name__ == '__main__':
    ...
    # Load the graph
        print(f'STEP 1 - Loading graph and building igraph object for {subset}...')
        Gm = get_graph(exp, subset, out_dir)
        print('-----DONE!-----')

        # Find df_peaks with embedding
        df_path = sorted(out_dir.glob(f'*_embed_{subset}.csv'))[-1]
        df_peaks = pd.read_csv(df_path, header=0, index_col=0)

        # Cluster the data and append info to df peaks
        print(f'STEP 2 - Clustering the data for {subset}...')
        df_peaks, res = cluster_leiden(Gm, df_peaks, res, fig_dir, exp, subset)
        df_peaks.to_csv(f'{out_dir}/{df_path.stem}_clust.csv')
        print('-----DONE!-----')

        # Plot the clusters
        print(f'STEP 3 - Plot clusters for {subset}...')
        plot_clusters(df_peaks, res, fig_dir, exp, subset)
        print('-----DONE!-----')