...
def peaks_in_cluster(clusters, matrix, bed_dir, exp, subset, genome_file):
    ...
    Args:
        ...
        subset (string): Subset of peaks (eg TE).
    ...
    for cluster in ['all'] + clusters:
        ...
        merged_peaks.saveas(f'{bed_dir}/{exp}_merged_peaks_{subset}_cluster_{cluster}.bed')
...
def plot_clustmap(cluster_rank, index, out_dir, exp, subset, enrich_thresh, types):
    ...
    for cluster, rank in cluster_rank.items():
        ...
        for i in index:
            ...
            if len(series) != 0:
                ...
                fisher_sorted_df = pd.read_csv(f'{out_dir}/{exp}_{types}_rank_{subset}_cluster_{cluster}.csv',
    ...
    enrichment_scores_not_all = enrichment_scores_not_all[enrichment_scores_not_all.columns[enrichment_scores_not_all.max() > 0]]
...
if __name__ == '__main__':
    ...
    subsets = ['all', 'TE']
    # No TSS or KZFP info for mouse
    if species == 'human':
        #subsets.extend([f'TE{tss_dist}', 'TEaway'])
        ...
        te_metadata = pd.read_csv(f'{utils_dir}/mm10_te_info.csv')
    ...
    for subset in subsets:
        ...
        print(f'____STARTING ANALYSIS FOR SUBSET {subset}____')
        print('STEP 1 - Retrieving necessary files...')

        matrix_path = sorted(out_dir.glob(f'*_{subset}_clust.csv'))[0]
        ...
        if species == 'human':
            df_clust = df_clust.drop(labels=[f'transcripts_{tss_dist}',
                                            f'TSS_{tss_dist}',
                                            'kzfps',
                                            'countkzfps',
                                            'kzfp_geneid',
        ...
        score_clustmap.savefig(f'{fig_dir}/avg_score_heatmap_{subset}.jpeg')
        print('-----DONE!-----')

        print('STEP 4 - Creating BED files of peaks for each cluster...')
        cluster_peaks = peaks_in_cluster(clusters, matrix, bed_dir, exp, subset, genome_file)
        print('-----DONE!-----')

        print('STEP 5 - Extracting all TE subfamilies which appear in subset...')
        ...
        for cluster, merged_peaks in cluster_peaks.items():
            ...
            fisher_df_sorted_te.to_csv(f'{out_dir}/{exp}_te_rank_{subset}_cluster_{cluster}.csv')
            print('-----DONE!-----')
            ...
            fig.write_html(f'{fig_dir}/{exp}_te_enrich_{subset}_cluster_{cluster}_thresh_{enrich_thresh_te}.html')
            print('-----DONE!-----')
        ...
        te_enrichment_scores, te_enrichment_scores_not_all, te_clustmap = plot_clustmap(cluster_ranked_tes, te_subfamilies, out_dir, exp, subset, enrich_thresh_te, types='te')
        te_enrichment_scores.to_csv(f'{out_dir}/{exp}_te_enrich_{subset}_clustmap_scores_thresh_{enrich_thresh_te}_full.csv')
        te_enrichment_scores_not_all.to_csv(f'{out_dir}/{exp}_te_enrich_{subset}_clustmap_scores_thresh_{enrich_thresh_te}_small.csv')
        te_clustmap.savefig(f'{fig_dir}/{exp}_te_clustmap_thresh_{enrich_thresh_te}_{subset}.jpeg')
        print('-----DONE!-----')

        if species == 'human':
            print('STEP 10 - Extracting all KZFP which appear in subset...')
            # Create a list of all entries in the element column of df
            ...
            for cluster, merged_peaks in cluster_peaks.items():
                ...
                fisher_df_sorted_kzfp.to_csv(f'{out_dir}/{exp}_kzfp_rank_{subset}_cluster_{cluster}.csv')
                print('-----DONE!-----')
                ...
                fig.write_html(f'{fig_dir}/{exp}_kzfp_enrich_{subset}_cluster_{cluster}_thresh_{enrich_thresh_kzfp}.html')
                print('-----DONE!-----')
            ...
            kzfp_enrichment_scores, kzfp_enrichment_scores_not_all, kzfp_heatmap = plot_clustmap(cluster_ranked_kzfps, kzfp_present, out_dir, exp, subset, enrich_thresh_kzfp, types='kzfp')
            kzfp_enrichment_scores.to_csv(f'{out_dir}/{exp}_kzfp_enrich_{subset}_clustmap_scores_thresh_{enrich_thresh_kzfp}_full.csv')
            kzfp_enrichment_scores_not_all.to_csv(f'{out_dir}/{exp}_kzfp_enrich_{subset}_clustmap_scores_thresh_{enrich_thresh_kzfp}_small.csv')
            kzfp_heatmap.savefig(f'{fig_dir}/{exp}_kzfp_clustmap_thresh_{enrich_thresh_kzfp}_{subset}.jpeg')
            print('-----DONE!-----')

        print(f'!!!____FINISHED ANALYSIS FOR SUBSET {subset}____!!!')
    print('____FINISHED EVERYTHING____')